...
def run_linear_sim(n_rules_grid, max_depth_grid, make_model,
                   n=100, p=10, s=2, sigma=0.1, n_iter=20):
    def f(x):
        p = x.shape[1]
        beta = np.array([1] * s + [0] * (p - s))
        return x @ beta
...
for comb in tqdm(param_combinations):
    ...
    sigma = comb["sigma"]
    s = comb["s"]
    results["vals"] = run_linear_sim(n_rules_grid, max_depth_grid,
                                     make_model, s=s, sigma=sigma, n_iter=50)
    results["sigma"] = sigma
    results["s"] = s
    results_list.append(results)

with open("../results/lin_sim_results.pkl", "wb") as file:
