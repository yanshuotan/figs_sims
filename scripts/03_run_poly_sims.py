...
from sim_util_tiffany import hierarchical_poly


def run_poly_sim(n_rules_grid, max_depth_grid, make_model,
                n=100, p=10, m=1, r=2, sigma=0.1, n_iter=20):

    def f(x):
        return hierarchical_poly(x, 0, m, r, beta=5)
...
for comb in tqdm(param_combinations):
    ...
    m = comb["m"]
    r = comb["r"]
    results["vals"] = run_poly_sim(n_rules_grid, max_depth_grid,
...
with open("../results/poly_sim_results.pkl", "wb") as file:
